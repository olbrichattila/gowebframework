...
func CreateMiddleware(a args.CommandArger, c commandcreator.CommandCreator, cc wizard.ClassCreator) {
	cc.SetHelpHeader("Usage:\n  go run ./cmd/ artisan create:middleware <middleware-name> <optional-parameters>\n")
	...
	err := c.Create(template, "./app/middlewares", templateParams)
	if err != nil {
		fmt.Printf("%s\nTry -help\n", err.Error())
		return
	}

	fmt.Printf("Please register your new middleware in:\n  app/config/middlewares.go\n")
}
